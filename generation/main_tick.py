...
def gen(task_data):
    ...
    if ("item_sacrifice" in task_data):
        f.write(item_sacrifice.tick_update("main",task_data))
        item_sacrifice.extra("main", task_data)
    
    #check
    f.write("\n#run again on failed conditions\n")
    #voting
    if ("voting" in task_data):
        f.write(voting.tick_check("main", task_data))
        voting.extra("main",task_data)
    #item sacrifice
    if ("item_sacrifice" in task_data):
        f.write(item_sacrifice.tick_check("main",task_data))
    #advancements
    if ("advancements" in task_data):
        f.write(advancement.tick_check("main",task_data))
