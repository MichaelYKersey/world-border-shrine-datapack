...
execute if score temp0 wbs.reg > temp1 wbs.reg run return run tellraw @a {"text":"An world boarder size tracking error occurred","color":"red"}
execute if score temp0 wbs.reg < temp1 wbs.reg run tellraw @a {"text":"A portion of the fog has dispersed","color":"#00cc44"}
