...
execute as @e[type=minecraft:interaction,tag=world_boarder_shrine] at @s as @a[distance=..10] run tellraw @s {"text":"strange there is such diversity of fish and objects distributed uniformly through the surrounding waters. I'll analysis the samples you brought.","color":"#9933ff"}
