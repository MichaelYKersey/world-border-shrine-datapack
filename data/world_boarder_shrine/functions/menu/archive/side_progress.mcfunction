scoreboard players operation temp0 wbs.reg = @s wbs.main_archive.progress
scoreboard players enable @s wbs.main_archive.progress
...
execute if score temp1 matches 0 wbs.vars run return run tellraw @s {"text":"Requested task is not avalible","color":"red"}
execute store result storage minecraft:world_boarder_shrine param.item int 1 run scoreboard players get temp0 wbs.reg
