...
execute as @e[type=minecraft:interaction,tag=world_boarder_shrine] at @s as @a[distance=..10] run tellraw @s {"text":"Build a station to monitor local weather. This info is necessary for continued revival of the world. It also must \"combat\" the fog somehow. The starting chemicals will only last so long.","color":"#9933ff"}
#print details
