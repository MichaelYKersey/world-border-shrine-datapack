...
execute as @e[type=minecraft:interaction,tag=world_boarder_shrine] at @s as @a[distance=..10] run tellraw @s {"text":"The weather manipulation system should be up and operational in a bit. Given the right resources it should temporarily change the weather.","color":"#9933ff"}
